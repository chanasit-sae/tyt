/*
 * EXAM Q3 — Two buttons: one toggles an LED, one prints the ADC value
 * GOAL:   Button A held   -> LED on (released -> off).
 *         Button B held   -> print the current ADC value to the serial terminal.
...
int main(void) {
    ...
    while (1) {                                     // poll the buttons (lab 1.3 style)
        // Button A: pressed -> LED on, released -> LED off
        if ((GPIOB->IDR & (1 << 4)) == 0)
            GPIOA->ODR |=  (1 << 5);
        else
            GPIOA->ODR &= ~(1 << 5);

        // Button B: while pressed, print the ADC value
        if ((GPIOB->IDR & (1 << 5)) == 0) {
            sprintf(buf, "ADC = %d\r\n", adc_read());
            uart_print(buf);
        }
